...
exports.transportation_view_one_Page = async function(req, res) {
    ...
    exports.transportation_create_Page = function(req, res) {
    console.log("create view")
    try{
    res.render('transportationcreate', { title: 'Transportation Create'});
    }
    catch(err){
    res.status(500)
    res.send(`{'error': '${err}'}`);
...
exports.transportation_update_Page = async function(req, res) {
    console.log("update view for item "+req.query.id)
    try{
    let result = await Transportation.findById(req.query.id)
    res.render('transportationupdate', { title: 'Transportation Update', toShow: result });
    }
    catch(err){
    res.status(500)
    res.send(`{'error': '${err}'}`);
...
exports.transportation_delete_Page = async function(req, res) {
    console.log("Delete view for id " + req.query.id)
    try{
    result = await Transportation.findById(req.query.id)
    res.render('transportationdelete', { title: 'Transportation Delete', toShow:
    result });
    }
    catch(err){
    res.status(500)
